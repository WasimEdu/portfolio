...
const FileContent = ({ className, isDivider = false, children, ...props }: FileContentProps) => {
    ...
    return (
        <div
            className={cn(
                "relative flex h-full min-h-0 gap-6 overflow-y-auto rounded-lg border bg-white/25 p-6 select-text",
                className,
            )}
            onPointerDownCapture={handlePointerDownCapture}
            {...props}
...
const FileInfo = ({ className, ...props }: React.ComponentProps<"div">) => {
    return <div className={cn("flex w-[350px] shrink-0 flex-col gap-2", className)} {...props} />;
};
...
const FileImage = ({ className, ...props }: React.ComponentProps<"div">) => {
    return (
        <div className="min-h-[220px] flex-1 -rotate-[2deg] border bg-zinc-100 p-4">
            <div
